import type { AnalysisResult, PrNeedingReview, PrWaitingOnAuthor, PrApproved, PrSizeInfo, PrAction, SummaryStats, StalenessConfig, DependencyGraph, PipelineStatus, PolicyStatus } from "../types.js";
...
function renderPrRow(
  ...
  const policyText = policyStatus ? ` ${formatPolicyBadge(policyStatus)}` : "";
  const pipelineText = !policyStatus && pipelineStatus ? ` ${formatPipelineBadge(pipelineStatus)}` : "";

  return `  ${ageText}  ${pad(prLink, 80)} ${authorText}${sizeText} ${actionText}${conflict}${policyText}${pipelineText}${stalenessText}${labelsText}`;
}
...
function renderSection<T>(
  ...
  bg: string,
  items: T[],
  getRow: (item: T) => { id: number; title: string; author: string; url: string; date: Date; hasMergeConflict: boolean; action: PrAction; size?: PrSizeInfo; detectedLabels?: string[]; stalenessBadge?: string | null; pipelineStatus?: PipelineStatus; isStarred?: boolean; policyStatus?: PolicyStatus },
  now: Date,
  ...
  if (getRepo) {
    ...
    for (const [repo, repoItems] of groups) {
      ...
      for (const item of repoItems) {
        const { id, title, author, url, date, hasMergeConflict, action, size, detectedLabels, stalenessBadge, pipelineStatus, isStarred, policyStatus } = getRow(item);
        lines.push(renderPrRow(id, title, author, url, date, hasMergeConflict, now, action, size, detectedLabels, stalenessBadge, pipelineStatus, isStarred, policyStatus));
      }
      lines.push("");
    }
  } else {
...
export function renderDashboard(options: RenderDashboardOptions): string {
  ...
  lines.push(renderSection("✅ Approved", BG_GREEN, approved,
    (pr: PrApproved) => ({ id: pr.id, title: pr.title, author: pr.author, url: pr.url, date: pr.createdDate, hasMergeConflict: pr.hasMergeConflict, action: pr.action, size: pr.size, detectedLabels: pr.detectedLabels, stalenessBadge: computeStalenessBadge(pr.createdDate, stalenessThresholds, now), pipelineStatus: pr.pipelineStatus, isStarred: pr.isStarred, policyStatus: pr.policyStatus }),
    now, repoGetter));

  lines.push(renderSection("👀 Needing Review", BG_YELLOW, needingReview,
    (pr: PrNeedingReview) => ({ id: pr.id, title: pr.title, author: pr.author, url: pr.url, date: pr.waitingSince, hasMergeConflict: pr.hasMergeConflict, action: pr.action, size: pr.size, detectedLabels: pr.detectedLabels, stalenessBadge: computeStalenessBadge(pr.waitingSince, stalenessThresholds, now), pipelineStatus: pr.pipelineStatus, isStarred: pr.isStarred, policyStatus: pr.policyStatus }),
    now, repoGetter));

  lines.push(renderSection("✍️  Waiting on Author", BG_RED, waitingOnAuthor,
    (pr: PrWaitingOnAuthor) => ({ id: pr.id, title: pr.title, author: pr.author, url: pr.url, date: pr.lastReviewerActivityDate, hasMergeConflict: pr.hasMergeConflict, action: pr.action, size: pr.size, detectedLabels: pr.detectedLabels, stalenessBadge: computeStalenessBadge(pr.lastReviewerActivityDate, stalenessThresholds, now), pipelineStatus: pr.pipelineStatus, isStarred: pr.isStarred, policyStatus: pr.policyStatus }),
    now, repoGetter));
